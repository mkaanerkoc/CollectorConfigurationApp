...
{
    public partial class LoRaTestiPage : UserControl, IEthernetToPagesInterface
    {
        ...
        {
            InitializeComponent();
            EthernetManager.Instance.SetPageInterface(Application_TypesConstants.LoRaTestPageID, this);
        ...
        private void sendLoRaPackageBtn_Click(object sender, EventArgs e)
        {
            ...
            EthernetManager.Instance.SendRemoteDeviceCmd(Ethernet_MessageIDs_t.INCOMING_CMD_LORA_SEND_PACKAGE_REQUEST, (ushort)(msgLength + 9), packageData);
        }

        private void parseNeighbourList(byte[] buffer)
        {
            ...
            for (int i = 0; i < neighbourCount; i++)
            {
                ...
                newNeighbourElement[0] = ((UInt16)(buffer[6 + i * NEIGHBOUR_ELEMENT_SIZE] << 8 | buffer[7 + i * NEIGHBOUR_ELEMENT_SIZE])).ToString();
                newNeighbourElement[2] = ((Int16)(buffer[8 + i * NEIGHBOUR_ELEMENT_SIZE] << 8 | buffer[9 + i * NEIGHBOUR_ELEMENT_SIZE])).ToString();
                newNeighbourElement[1] = ((UInt16)(buffer[13 + i * NEIGHBOUR_ELEMENT_SIZE])).ToString();
                newNeighbourElement[3] = string.Format("{0:00}:{1:00}:{2:00}", buffer[10 + i * NEIGHBOUR_ELEMENT_SIZE], buffer[11 + i * NEIGHBOUR_ELEMENT_SIZE], buffer[12 + i * NEIGHBOUR_ELEMENT_SIZE]);
                neighborListView.Invoke((MethodInvoker)delegate
                {
        ...
        private void parseNewReceivedPackage(byte[] buffer)
        {

            int messageLengthParam = (( buffer[3] << 8 ) | ( buffer[4] ) + 8);
        ...
        private void sendReceiveLoRaPackageCmdBtn_Click(object sender, EventArgs e)
        {
            ...
            outputData[0] = 2; // ETHERNET
            if (receiveLoRaRxPkgFromUARTCb.Checked)
                outputData[1] = 1;
            else
                outputData[1] = 0;
            EthernetManager.Instance.SendRemoteDeviceCmd(Ethernet_MessageIDs_t.INCOMING_CMD_LORA_ECHO_MONITOR_SET_REQUEST, 2, outputData);
        }
        ...
        private void sendGetEchoSettingsBtn_Click(object sender, EventArgs e)
        {
            ...
            outputData[0] = 2; // ETHERNET
            EthernetManager.Instance.SendRemoteDeviceCmd(Ethernet_MessageIDs_t.INCOMING_CMD_LORA_ECHO_MONITOR_GET_REQUEST, 1, outputData);
        }

        private void getNeighbourListBtn_Click(object sender, EventArgs e)
        {
            byte[] outputData = new byte[0];
            EthernetManager.Instance.SendRemoteDeviceCmd(Ethernet_MessageIDs_t.INCOMING_CMD_NEIGHBOUR_LIST_GET_REQUEST, 0, outputData);
        }
        ...
        private void removeReceivedPackageBtn_Click(object sender, EventArgs e)
        {
